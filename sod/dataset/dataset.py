...
def get_loader(args, mode='train', pin=False):
    shuffle = False
    if mode == 'train':
        shuffle = True
        dataset = ImageDataTrain(args.train_root, args.train_list)
        data_loader = data.DataLoader(dataset=dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=args.num_thread, pin_memory=pin)
    else:
        dataset = ImageDataTest(args.test_root, args.test_list)
        data_loader = data.DataLoader(dataset=dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=args.num_thread, pin_memory=pin)
    return data_loader
